...
| Vision AI primary | GPT-4o | Best accuracy for fridge reading and Indian dish identification |
| Vision AI fallback | Gemini 2.0 Flash | 10x cheaper, used for simple Meal Snap calls post-launch |
| Recipe engine | Claude Sonnet 4 | Best structured JSON output, strong Indian cuisine knowledge |
| Nutrition estimation | Claude Sonnet 4 | Same model, different prompt for unknown dishes |
...
└───┬──────────────┬──────────────┬───────┘
    │              │              │
┌───▼───┐    ┌─────▼─────┐  ┌────▼───────────────┐
│GPT-4o │    │Claude     │  │ Swiggy MCP          │
│Vision │    │Sonnet 4   │  │ Instamart + Food    │
...
**Key libraries:**
- `openai` — GPT-4o vision calls (base64 image → ingredient/dish JSON)
- `anthropic` — Claude Sonnet 4 for recipe engine and nutrition estimation
...
- Higher accuracy on complex scenes (cluttered fridge, mixed thali)
- Handles ambiguous Indian dishes better in testing
- Cost: ~$0.01–0.03 per image at current pricing

**Fallback model:** Gemini 2.0 Flash (vision)
- 10x cheaper than GPT-4o
- Slightly lower accuracy on edge cases
- Used when: GPT-4o is unavailable, or user has hit a daily free snap limit

**Routing logic:**
- Mode 1 (FridgeScan): always use GPT-4o. This is a high-stakes call (ordering real groceries). Accuracy matters more than cost.
- Mode 2 (Meal Snap): use Gemini Flash by default. Switch to GPT-4o if confidence score is below 70%.
...
**Why Claude specifically:** Superior instruction-following for structured output, strong on Indian cuisine knowledge, cost-effective for recipe generation
...
**Mitigation:**
- Use GPT-4o (most capable vision model currently available) for Mode 1
...
The Expo (React Native) + FastAPI + GPT-4o + Claude Sonnet 4 stack demonstrates real engineering capability across mobile, backend, and AI. It's not a chatbot wrapper or a web form. It uses multiple Swiggy APIs in a deliberate, sequenced flow with a fully native camera interface — showing genuine understanding of both the platform and the product.
